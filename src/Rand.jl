...
if isfile("Rand/num_dir")
    num_dir = convert(Int64,readdlm("Rand/num_dir")[1])
else
    num_dir = 0
    writedlm("Rand/num_dir",num_dir)
end
...
while ~finished
    ...
    num_dir += 1
    mkdir("Rand/$num_dir")
    writedlm("Rand/$num_dir/rate_constant.dat",rate)
    writedlm("Rand/$num_dir/saturation_constant.dat",sat)
    writedlm("Rand/$num_dir/control_constant.dat",cont)
    writedlm("Rand/$num_dir/Tsim",Tsim)
    writedlm("Rand/$num_dir/X",X)
    writedlm("Rand/$num_dir/cost",cost)
    writedlm("Rand/$num_dir/KE",KE)
    writedlm("Rand/num_dir",num_dir)
end
